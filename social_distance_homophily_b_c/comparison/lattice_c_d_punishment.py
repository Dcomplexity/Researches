...
def initialize_population():
    ...
    for i in range(total_num):
        popu.append(Agent(i, network[i], np.random.choice([0, 1, 2, 3], p=[0.25, 0.25, 0.25, 0.25])))
...
def find_neigh_defectors(popu, network, total_num):
    ...
    for i in range(total_num):
        for j in network[i]:
            if popu[j].get_strategy() == 0 or popu[j].get_strategy() == 1:
...
if __name__ == "__main__":
    ...
    file_name = dir_name + 'frac_co_comparison_lattice_c_d_punishment_cost_%s_d_%s.csv' % (punishment_cost_r, defect_param_r)
    f = open(file_name, 'w')
