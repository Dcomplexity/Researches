...
def run_game(step, ind_rep, ind_strategy, alpha, beta, play_num, defect_param, group_size, group_base, group_length, total_num,
    ...
    group_rep = build_rep(old_ind_strategy, pos_ind, group_base, group_length)
    for i in range(total_num):
        ind_rep[i] = ind_rep[i] + gamma * (group_rep[ind_pos[i]] - ind_rep[i])
...
if __name__ == "__main__":
    ...
    dir_name = abs_path + '/results/re_history_reputation/'
    if not os.path.isdir(dir_name):
        os.makedirs(dir_name)
    file_name = dir_name + 'frac_co_history_reputation_gs_%s_d_%s_g_%s.txt' % (group_size_r, defect_param_r, gamma_r)
    f = open(file_name, 'w')
