...
def evolution_one_step(popu, total_num, edges):
    # Play the game
    # for i in range(total_num):
    ...
    for edge in edges:
        i = edge[0]
        j = edge[1]
        r_i, r_j = pd_game_0(popu[i].get_strategy(), popu[j].get_strategy())
...
def run():
    ...
    for _ in range(run_time):
        popu = evolution_one_step(popu, total_num, edges)
    return popu, network, total_num, edges,


def evaluation(popu, edges):
    sample_time = 20
    sample_strategy = []
    total_num = len(popu)
...
if __name__ == "__main__":
    ...
    for _ in range(initializations):
        population, network_v, total_number_v, edges_v = run()
        result.append(evaluation(population, edges_v))
