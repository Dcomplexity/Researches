...
double gammaValue = 0.99;

double getAlpha(int timeStep) {
...
int find_min_index(double *array) {
    int length = int(sizeof(array) / sizeof(*array));
...
int chooseFromDis(double* distribution) {
    srand(unsigned(time(0)));
    double randomW = (double) rand() / RAND_MAX;
    double sumW = 0.0;
    for (int disI = 0; disI <= int(sizeof(distribution)/sizeof(*distribution)); disI++) {
...
int main(int argc, char* argv[]) {
    ...
    double sumAccount[staNum][actNum];
    double avgFrac[staNum][actNum];

    Node *p = NULL;
    Node *q = NULL;

    double curAction[nodenumber];
    double preAction[nodenumber];

    double curStrategyFrac[nodenumber][staNum][actNum];
    double preStrategyFrac[nodenumber][staNum][actNum];

    double curState[nodenumber];
    double preState[nodenumber];
