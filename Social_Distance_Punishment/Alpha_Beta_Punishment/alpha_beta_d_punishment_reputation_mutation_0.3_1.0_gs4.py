...
def initializeStrategy(totalNum):
    indStrategy = np.random.choice([0, 1, 2, 3], totalNum, p=[0.25, 0.25, 0.25, 0.25])
    return indStrategy
...
def find_defectors(indStrategy, posInd, groupBase, groupLength):
    ...
    for i in range(positionNum):
        for j in posInd[i]:
            if indStrategy[j] == 0 or indStrategy[j] == 1:
...
def runGame(indStrategy, alpha, beta, playNum, defectParam, groupSize, groupBase, groupLength, totalNum, indPos, posInd, rt, rq):
    ...
    for i in range(totalNum):
        ...
        if w1 > w2:
            ...
            potential_strategy.remove(oldIndStrategy[i])
            indStrategy[i] = np.random.choice(potential_strategy)
        else:
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(dirname):
        os.makedirs(dirname)
    filename = dirname + "Re_Alpha_Beta_D_Punishment_Reputation_Mutation_0.3_1.0_Gs_4_Dp_%s_Rt_%s_Rq_%s.csv" % (buildDefectParam, buildRt, buildRq)
    f = open(filename, 'w')
