...
def runGame(indStrategy, alpha, beta, playNum, defectParam, groupSize, groupBase, groupLength, totalNum, indPos, posInd, rt):
    """
    Run one game
    ...
    for i in range(totalNum):
        ...
        repFre = 1 / (1 + rt * math.e ** -(indRep[playerIndex] * indRep[opponentIndex] * 8))
...
if __name__ == "__main__":
    ...
    dirname = abspath + "/Results/Re_Reputation_Mutual_Time/Re_Groupsize_2/"
    if not os.path.isdir(dirname):
        os.makedirs(dirname)
    filename = dirname + "Re_Co_Rate_Gs_2_Dp_%s.txt" %buildDefectParam
    f = open(filename, 'w')
        ...
        for buildBeta in range(-3, 4):
            ...
            for roundIndex in range(rounds):
                ...
                for i in range(runtime):
                    buildIndStrategy = runGame(buildIndStrategy, buildAlpha, buildBeta, buildPlayNum, buildDefectParam, buildGroupSize, buildGroupBase, buildGroupLength, buildTotalNum, buildIndPos, buildPosInd, i)
                sampleStrategy = []
                for i in range(sampletime):
                    buildIndStrategy = runGame(buildIndStrategy, buildAlpha, buildBeta, buildPlayNum, buildDefectParam, buildGroupSize, buildGroupBase, buildGroupLength, buildTotalNum, buildIndPos, buildPosInd, i+runtime)
