...
def initailizeStrategy(totalNum):
    indStrategy = np.random.choice([0, 1], totalNum, p=[0.2, 0.8])
    return indStrategy
...
def runGame(indStrategy, alpha, beta, playNum, defectParam, groupSize, groupBase, groupLength, totalNum, indPos, posInd):
    ...
    for i in range(totalNum):
        ...
        individual_pool = list(posInd[nowPosition][:])
        individual_pool.remove(i)
        potential_indvidual = [x for x in individual_pool]
        # potentialPos = getPosition(groupLength, nowPosition, probPlay)
        opponentPlay[i] = np.random.choice(potential_indvidual)

    # generate the opponent from whom learn
    for i in range(totalNum):
        # nowPosition = indPos[i]
        # potentialPos = getPosition(groupLength, nowPosition, probLearn)
        opponentLearn[i] = np.random.choice(range(0, totalNum))
...
if __name__ == "__main__":
    ...
    dirname = abspath + "/Results/Re_Investigate_Reason/"
    if not os.path.isdir(dirname):
        os.makedirs(dirname)
    filename = dirname + "Re_Co_Rate_Gs_4_n_%s_Dp_%s.txt" %(buildTotalNum, buildDefectParam)
    f = open(filename, 'w')
