...
def pickIndividualBasedRep(focInd, positionRep, positions, posInd, indPos):
    """
    ...
    :param
    focInd: the individual to pick up opponent
    positions: the positions from which pick up an individual
    posInd: position -> individual
    indPos: individual -> position
    :return
    indIndex (int): the index of individual
    """
    focRep = positionRep[indPos[focInd]]
    potentialInd = []
    potentialPosRep = []
    for i in positions:
...
def runGame(indStrategy, alpha, beta, playNum, defectParam, groupSize, groupBase, groupLength, totalNum, indPos, posInd):
    ...
    for i in range(totalNum):
        ...
        opponentPlay[i] = pickIndividualBasedRep(i, groupRep, potentialPos, posInd, indPos)
...
if __name__ == "__main__":
    ...
    dirname = abspath + "/Results/Reputation_Distance_bo/One_Dimension_GroupSize_4/"
    if not os.path.isdir(dirname):
        os.makedirs(dirname)
    filename = dirname + "Co_Rate_GroupSize_4_DefectParam_%s.txt" %buildDefectParam
    f = open(filename, 'w')
