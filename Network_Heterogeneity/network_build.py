...
def get_other_position(group_length, now_position, distance_prob):
    potential_pos = []
    distance = np.random.choice(group_length, 1, p=distance_prob)[0] + 1
    if distance == 1:
        potential_pos.append(now_position)
...
def new_structure(pos_ind, group_size, dimension_distance):
...
def new_other_structure(pos_ind, ind_pos, group_length, group_size, reg_param):
    len_pos = len(pos_ind)
    len_ind = len(ind_pos)
    ...
    for i in range(len_ind):
        ...
        potential_pos = get_other_position(group_length, now_position, distance_prob)
...
def build_multi_pos_ind(mul_dimen, pos_ind, group_size, dimension_distance):
    ...
    for i in range(mul_dimen):
        pos_ind_mul.append(new_structure(pos_ind, group_size, dimension_distance))
    return pos_ind_mul
...
def generate_network(mul_dimen, degree, group_size, group_base, group_length, dimen_l, alpha):
    ...
    pos_ind_mul = []
    for i in range(mul_dimen):
        pos_ind_mul.append(new_structure(pos_ind, group_size, dimen_l))
    ...
    while edge_count < edge_num:
        ...
        potential_pos = get_position(group_length, i_pos, distance_prob)
...
def generate_network_in_paper(mul_dimen, degree, group_size, group_base, group_length, alpha, beta):
    total_num = group_size * (group_base ** (group_length - 1))
    edge_num = degree * total_num / 2
    ind_pos, pos_ind = build_structure(group_size, group_base, group_length)
    pos_ind_mul = []
    for i in range(mul_dimen):
        pos_ind_mul.append(new_other_structure(pos_ind, ind_pos, group_length, group_size, beta))
    ind_pos_mul = build_multi_ind_pos(pos_ind_mul, total_num)
    distance_prob = generate_distance_prob(group_length, group_size, alpha)
    adj_link = {}
    edge_count = 0
...
def generate_network_connected(mul_dimen, degree, group_size, group_base, group_length, dimen_l, alpha):
    adj_link = generate_network(mul_dimen, degree, group_size, group_base, group_length, dimen_l, alpha)
    G = nx.Graph(adj_link)
    gen_num = 0
...
def generate_network_in_paper_connected(mul_dimen, degree, group_size, group_base, group_length, alpha, beta):
    adj_link = generate_network_in_paper(mul_dimen, degree, group_size, group_base, group_length, alpha, beta)
    G = nx.Graph(adj_link)
    gen_num = 0
    while not nx.is_connected(G):
        print(gen_num)
...
if __name__ == "__main__":
    ...
    G_r = generate_network_connected(mul_dimen_r, degree_r, group_size_r, group_base_r, group_length_r, dimen_l_r, alpha_r)
    # G_ip_r = generate_network_in_paper_connected(mul_dimen_r, degree_r, group_size_r, group_base_r, group_length_r, alpha_r, beta_r)
    # G_hete_r = generate_hete_network_connected(mul_dimen_r, degree_r, group_size_r, group_base_r, group_length_r, alpha_r)
    G = G_r
    if G != None:
