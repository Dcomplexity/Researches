...
def buildIndividualRep(indStrategy, posInd, groupBase, groupLength, totalNum, indPos):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--normParam', type=float, required=True,
                        help="This Parameter defines the influence of Reputaion on Tag")
    ...
    if not os.path.isdir(dirname):
        os.makedirs(dirname)
    filename = dirname + "Re_Tag_0_1_uniform_Gs_8_c_%s_Dp_%s_t_%s.txt" % (buildC, buildDefectParam, buildTolerance)
    f = open(filename, 'w')
        ...
        for buildBeta in range(-3, 4):
            ...
            for roundIndex in range(rounds):
                ...
                for i in range(runtime):
                    ...
                    buildIndRep = buildIndividualRep(buildIndStrategy, buildPosInd, buildGroupBase, buildGroupLength, buildTotalNum, buildIndPos)
                    for ii in range(buildTotalNum):
                        buildIndTag[ii] = buildIndTag[ii] + (buildIndRep[ii] - buildIndTag[ii]) * buildC
                sampleStrategy = []
