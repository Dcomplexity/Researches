...
if __name__ == "__main__":
    ...
    dirname = abspath + "/Results/Re_Per_Tag_Frac_Co/"
    if not os.path.isdir(dirname):
        os.makedirs(dirname)
    filename = dirname + "Re_Per_Tag_10_uniform_Gs_8_Dp_%s_t_%s.csv" %(buildDefectParam, buildTolerance)
    ...
    for buildAlpha in range(-2, -1):
        for buildBeta in range(2, 3):
            ...
            for roundIndex in range(rounds):
                ...
                indStrategyTime = []
                indStrategyTime.append(buildIndStrategy.copy())
                ...
                for i in range(runtime):
                    ...
                    indStrategyTime.append(buildIndStrategy.copy())
    indStrategyTime.append(buildIndTag)
    indStrategyTimePd = pd.DataFrame(indStrategyTime)
    index_tag = list(np.arange(runtime+1)) + ['tag']
    print(index_tag)
    indStrategyTimePd.insert(0, 'index_tag', index_tag)
    indStrategyTimePd.to_csv(filename, index=None)
