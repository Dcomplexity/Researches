...
def initialize_strategy(total_num, initialize_frac_co):
    ind_strategy = np.random.choice([0, 1], total_num, p=[1-initialize_frac_co, initialize_frac_co])
    return ind_strategy
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f', '--initial_frac_co', type=float, required=True, help='Set the initial fraction of cooperators')
    args = parser.parse_args()
    defect_param_r = args.defect_param
    initial_frac_co_r = args.initial_frac_co
    play_num_r = 1
    abs_path = os.path.abspath(os.path.join(os.getcwd(), '../'))
    dir_name = abs_path + '/results/re_alpha_beta_initial_frac_co/'
    if not os.path.isdir(dir_name):
        os.makedirs(dir_name)
    file_name = dir_name + 'frac_co_alpha_beta_gs_%s_if_%s_d_%s.txt' % (group_size_r, initial_frac_co_r, defect_param_r)
    f = open(file_name, 'w')
    ...
    rounds = 10
    results_r = []
    for alpha_r in range(-3, 4):
        for beta_r in range(-3, 4):
            print(alpha_r, beta_r)
            round_results_r = np.zeros(rounds)
            for round_index in range(rounds):
                ind_strategy_r = initialize_strategy(total_num_r, initial_frac_co_r)
